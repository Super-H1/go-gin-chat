...
func mainProcess(c *websocket.Conn) {
	for {
		...
		log.Printf("来自客户端的消息", clientMsg, c.RemoteAddr())
		...
		if clientMsg.Status == msgRobot { // 消息发送
			serveMsgStr = formatServeMsgStr(msgRobot)
		}
		...
		if clientMsg.Status == msgRobot || clientMsg.Status == msgTypeOnline {
