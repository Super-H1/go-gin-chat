...
func write() {
	...
	for {
		...
		case cl := <-sMsg:
			...
			case msgRobot:
				notifyR(cl.Conn, string(serveMsgStr))
...
func notifyR(conn *websocket.Conn, msg string) {
	chNotify <- 1 // 利用channel阻塞 避免并发去对同一个连接发送消息出现panic: concurrent write to websocket connection这样的异常
	_, roomIdInt := getRoomId()
	assignRoom := rooms[roomIdInt]
	for _, con := range assignRoom {
		if con.(wsClients).Conn.RemoteAddr().String() == conn.RemoteAddr().String() {
			err := con.(wsClients).Conn.WriteMessage(websocket.TextMessage, []byte(msg))
			if err != nil {
